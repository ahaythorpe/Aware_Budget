...
struct HomeView: View {
    ...
    private var debugAuthBanner: some View {
        HStack(spacing: 8) {
            Image(systemName: SupabaseService.debugAuthStatus.contains("signed") ? "checkmark.circle.fill" : "exclamationmark.triangle.fill")
                .foregroundStyle(SupabaseService.debugAuthStatus.contains("signed") ? DS.positive : DS.warning)
            VStack(alignment: .leading, spacing: 2) {
                Text("DEBUG AUTH")
                    .font(.system(size: 9, weight: .heavy, design: .rounded))
                    .tracking(1.2)
                    .foregroundStyle(DS.textTertiary)
                Text(SupabaseService.debugAuthStatus)
